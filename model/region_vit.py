...
class MultiHeadTenporalAttention(nn.Module):
    r""" Multi-head temporal attention module with relative position bias for RSA and LSA.
    ...
    def __init__(self, dim, window_size, window_num, num_heads, qkv_bias=True, qk_scale=None, num_frames=3, attn_drop=0., proj_drop=0.):
        ...
        self.proj = nn.Linear(num_frames * dim, dim)
    ...
    def forward(self, x):
        ...
        """
        B_, N, C = x.shape
        mid = x.contiguous().view(-1, self.num_frames, N, C)[:, self.num_frames//2, :, :]
        B, _, _ = mid.shape

        q = self.q(mid).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        kv = self.kv(x).reshape(B, self.num_frames, N, 2, self.num_heads, C // self.num_heads).permute(3, 1, 0, 4, 2, 5)
        ks, vs = kv[0], kv[1]
        ...
        for k, v in zip(ks, vs): 
            ...
            qs.append((attn @ v).transpose(1, 2).reshape(B, N, C))

        rst = torch.cat(qs, dim=2)
        
        rst = self.proj(rst)
        rst = self.proj_drop(rst)
        rst = F.pad(rst.unsqueeze(dim=1), (0, 0, 0, 0, self.num_frames//2, self.num_frames//2))
        
        x = x.contiguous().view(-1, self.num_frames, N, C)
        x = x + rst
        x = x.contiguous().view(B_, N, C)
...
class R2LEncoderBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=8, window_num=(8, 8), num_frames=3,
        ...
        self.attn = MultiHeadSelfAttention(
            dim, window_size=to_2tuple(self.window_size), window_num=self.window_num, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.cross_attn = MultiHeadTenporalAttention(
            dim, window_size=to_2tuple(self.window_size), window_num=self.window_num, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, num_frames=num_frames)
    ...
    def forward(self, regional_tokens, x):
        ...
        regional_tokens = self.drop_path(self.cross_attn(self.norm4(regional_tokens)))
    ...
    def flops(self):
        ...
        H, W = self.input_resolution
        # norm1
        flops += self.dim * self.window_num[0] * self.window_num[1]
        # RSA
        flops += self.attn.flops(self.window_num[0] * self.window_num[1])
        # norm2
        flops += self.dim * (H * W + self.window_num[0] * self.window_num[1])
        # LSA
        nW = H * W / self.window_size / self.window_size
        flops += nW * self.attn.flops(self.window_size * self.window_size + 1)
        # mlp
        flops += 2 * (H * W + self.window_num[0] * self.window_num[1]) * self.dim * self.dim * self.mlp_ratio
        # norm3
        flops += self.dim * (H * W + self.window_num[0] * self.window_num[1])
        return flops
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, num_frames=3,
        ...
        self.window_size = window_size
        window_num = (input_resolution[0] // window_size, input_resolution[1] // window_size)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(64, 64), patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, method='3-conv'):
        super().__init__()
        ...
        if method == '3-conv':
            ...
            self.proj = nn.Conv2d(in_chans, embed_dim, 2*patch_size[0], patch_size[0], patch_size[0]//2)
        elif method == 'linear':
            self.proj = nn.Sequential(
...
class RegionViT(nn.Module):
    ...
    def __init__(self, img_size=(64, 64), patch_size=1, in_chans=128, num_frames=3,
                 num_classes=1000, embed_dim=256, depths=[4], num_heads=[4],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 local_tokenization_method='linear', regional_tokenization_method='linear',
        ...
        self.num_windows = (img_size[0] // patch_size // window_size) * (img_size[1] // patch_size // window_size)
        ...
        self.regional_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size*window_size, in_chans=in_chans, embed_dim=embed_dim,
        ...
        if patch_size == 1:
            self.head = nn.Conv2d(self.num_features, in_chans, kernel_size=1)
        else:
            self.head = nn.ConvTranspose2d(self.num_features, in_chans, stride=patch_size, padding=patch_size//2, kernel_size=patch_size*2)
    ...
    def forward_features(self, img):
        ...
        x = local_tokens.transpose(1, 2).contiguous().view(BF_, self.num_features, H_//self.patch_size, W_//self.patch_size)
        x = self.head(x)
#         x = x.transpose(1, 2).contiguous().view(BF_, C_, H_, W_)
#         x = x[:, F_//2, :, :, :]
