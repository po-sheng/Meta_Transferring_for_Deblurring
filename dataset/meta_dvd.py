    ...
    def __init__(self, args):
        ...
        if self.args.meta_train:
            if os.path.isdir(os.path.join(args.dataset_dir, 'train', video)):
                self.blur_list = sorted(glob.glob(os.path.join(args.dataset_dir, 'train', video, "input", '*.jpg')))
                self.sharp_list = sorted(glob.glob(os.path.join(args.dataset_dir, 'train', video, "GT", '*.jpg')))
        else:
            if os.path.isdir(os.path.join(args.dataset_dir, 'test', video)):
                self.blur_list = sorted(glob.glob(os.path.join(args.dataset_dir, 'test', video, "input", '*.jpg')))
                self.sharp_list = sorted(glob.glob(os.path.join(args.dataset_dir, 'test', video, "GT", '*.jpg')))
