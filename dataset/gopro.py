...
from .metaOperation import get_meta_data, self_shift, find_sharp
...
class TrainSet(Dataset):
    def __init__(self, args):
        ...
        self.blurrer_transform = transforms.Compose([RandomRotate(), RandomFlip(), Normalize(args.centralized, args.normalized), ToTensor()])
    ...
    def __getitem__(self, idx):
        ...
        # find blurrer patch from center frame
        if self.args.n_frames == 1:
            max_diff, max_img = float('-inf'), 0
#             for i in range(50):
#                 H = random.randint(0, self.args.img_h - self.args.input_h)
#                 W = random.randint(0, self.args.img_w - self.args.input_w)
#                 
#                 img = blurs[0][H:H+self.args.input_h, W:W+self.args.input_w, :]
#                 diff = self_shift(img)
#                 
#                 if diff > max_diff:
#                     max_diff = diff
...
#             blurrers.append(max_img)
        ...
        if self.transform:
            ...
            blurrers = self.blurrer_transform({'blurrers': blurrers})
         
        return sample, blurrers
