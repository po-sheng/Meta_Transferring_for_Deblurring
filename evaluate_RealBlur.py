...
#     file_path = "~/reblur_exp/exp/meta_test/GOPRO/reblur_exp_attn_mprnet_gan_0.99_cycle_update_n_10_patch_inner_e6_gopro_on_realblur_j_00005/save_results"
    file_path = "/home/psliu/reblur_exp/exp/meta_test/GOPRO/reblur_exp_attn_mimoPlus_gan_0.99_cycle_update_n_10_patch_gopro_on_realblur_j_00019/save_results"
    gt_path = "/disk1/psliu/datasets/benchMark/RealBlur/full_RealBlur_J/test/*/gt"

    path_list = natsorted(glob(os.path.join(file_path, '*_000*.png')) + glob(os.path.join(file_path, '*_000*.jpg')))
    ...
    print('Aligned metric result PSNR: {:f} SSIM: {:f}\n'.format(avg_psnr, avg_ssim))

    with open(os.path.join(file_path, "..", "test.log"), "a") as f:
        f.write("\n")
        f.write("Aligned metric result PSNR: {:f} SSIM: {:f}\n".format(avg_psnr, avg_ssim))
